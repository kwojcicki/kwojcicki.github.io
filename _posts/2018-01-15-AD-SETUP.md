...
Next we need to configure the bridged network that the virtual machine will be using. You can do this using CLI or using the network manager.  http://ask.xmodulo.com/configure-linux-bridge-network-manager-ubuntu.html provides a good visual example on how to do this using the network manager. You can ignore the steps about removing the wired connection.

sudo nm-connection-editor

click add → bridge

Fill in connection name and Interface name with whatever you want. 

In the general tab ensure both "Automatically connect to this network when it is available" and "All users may connect to this network" are checked

In the bridge page go to add → Ethernet 

Set device to your normal ethernet connection ie: enp0s25

In the general tab ensure both "Automatically connect to this network when it is available" and "All users may connect to this network" are checked

Click save for both the Ethernet window and the bridge window 
...
Before starting the virtual machine and selecting the ISO file. Go to the settings of the virtual machine → network → adapater 1 and selected Bridged Adapter with the Name that we created before.
...
Ensure that the virtual machine is connected to the lab with a 172.xxx ip address and has internet connectivity
...
Using powershell (experimental)
Download attached DeploymentConfigTemplate.xml and place into VM
...
Restart afterwards will probably be required
...
Using powershell (experimental)
Download attached DeploymentConfigTemplate1.xml and place into VM
...
Install-WindowsFeature -ConfigurationFilePath .ADCS-DeploymentConfigTemplate1.xml

Restart may be required afterwards 
...
Create a new request.inf definition with the following contents - replacing Subject with the qualified domain name of your active directory server (only need to change if didn't set host name to LOCAL-AD-TEST and root domain to
...
Testing regular AD on linux machine
TODO
