...
Let's take a look at a simple example to better understand Traefiks key concepts. Let's imagine we have a simple service called `whoami` that we are looking to expose to the outer world. `whoami` simply returns information about the machine it is deployed on.
...
Start the containers using `docker-compose up`. Then let's curl our `whoami` service.
...
Since any user is free to browse our `inventory`, those routes can be accessed without authorization. But for our all `order` and `account` requests the user must be authenticated. We can accomplish this using Traefiks middleware pattern.
...
But we can access our inventory without any need for authetnication.
...
<!-- Such inconsistencys can often happen with rapidily changing solutions, will try to fix this using Traefik while not deprecating any clients that may use the old `/order` endpoint. -->
...
<!-- Now one day our product manager comes along and tells us our customers are complaining that it takes too long to fetch the an orders status. So as engineers we do the only reasonable thing and redesign the api from scratch. But in order to keep backwards compatibility with older clients we need to keep the old api around.

Now lets assume we followed some not best practices when creating our API's and are working on upgrading our clients to a new endpoint. Previously we were calling our `order` service utilizing the path `/order`. Now will have two paths `/api/v1/order` and `/api/v2/order`. Since some of our old clients have yet to upgrade they will still be requesting `/order` and we need to accommodate them. -->
